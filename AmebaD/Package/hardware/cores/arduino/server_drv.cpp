...
bool ServerDrv::recvData(int sock, const uint8_t *data, uint16_t len) {
    int ret;
    int flag = 0;
    ...
    ret = receive_data(sock, data, len, flag);
    if (ret <= 0) {
        return false;
    }
    return true;
...
int ServerDrv::startClient(uint32_t ipAddress, uint16_t port, uint8_t protMode) {
    ...
    if(getIPv6Status() == 0){
        ...
        printf("\n\r[INFO]server_drv.cpp:  start_client_v6\n\r", sock);
        sock = start_client_v6(port, protMode);
    }
    return sock;
}
...
void ServerDrv::setIPv6TCPServer(void) {
    ipv6_tcp_server();
}

void ServerDrv::setIPv6UDPClient(void) {
    ipv6_udp_client();
}
